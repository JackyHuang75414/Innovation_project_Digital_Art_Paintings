...
export const useTradingStore = defineStore('trading', () => {
  ...
  // SSE handle for real-time VMM price stream
  ...
  function destroy() {
    if (_sseEmitter) { _sseEmitter.close(); _sseEmitter = null }
  }

  /** Connect to the backend VMM SSE stream for live price updates. */
  function _connectSse() {
    if (_sseEmitter) return
    const es = new EventSource('/api/vmm/stream')
    _sseEmitter = es

    es.addEventListener('vmm', (e) => {
      const data = JSON.parse(e.data)
      // Update in-memory prices reactively
      Object.entries(data.prices ?? {}).forEach(([id, p]) => {
        prices.value[Number(id)] = Number(p)
      })
      // Append new ticks to history
      ;(data.ticks ?? []).forEach(({ artworkId, t, p }) => {
        const h = history.value[Number(artworkId)]
        if (h) {
          h.push({ t, p: Number(p) })
    ...
    es.onerror = () => {
      ...
      _sseEmitter = null
      // Reconnect after 3 s
      setTimeout(() => { if (!_sseEmitter) _connectSse() }, 3000)
    }
  }
